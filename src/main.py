...
from services.egress_guard import guard as egress_guard
...
async def lifespan(app: FastAPI):
    ...
    # Target-egress policy for tenant probes (SSRF hardening). The guard vets
    # every connect against the blocked-range policy; force_tls_verify overrides
    # a script's rejectInvalidCerts=false. Both are on in production, off in dev
    # and the Compose/e2e stacks (which probe internal targets) unless set.
    egress_enabled = settings.egress_guard_enabled
    force_verify = settings.force_tls_verify_enabled
    init_egress_policy(egress_guard if egress_enabled else None, force_verify)
    log.info("egress guard: %s; force TLS verify: %s",
             "on" if egress_enabled else "off", "on" if force_verify else "off")
